...
class Configuration implements ConfigurationInterface
{
    ...
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $rootNode
                    ...
                    ->prototype('array')
                        ->children()
                            ...
                            ->arrayNode('serializer')
                                ...
                                ->end()
                                ->validate()
                                    ->ifTrue(static fn (array $config): bool => isset($config['context_builder_service']) && $config['context_mapping'])
                                    ->thenInvalid('You cannot use "context_builder_service" and "context_mapping" at the same time.')
                                ->end()
                            ->end()
                        ->end()
                        ->validate()
                            ->ifTrue(static fn (array $config): bool => isset($config['mapping_directory'], $config['mapping_provider_service']))
                            ->thenInvalid('You cannot use "mapping_directory" and "mapping_provider_service" at the same time.')
                        ->end()
                        ->validate()
                            ->ifTrue(static fn (array $config): bool => !isset($config['mapping_directory']) && !isset($config['mapping_provider_service']))
            ...
            ->end()
            ->validate()
                ->ifTrue(static fn (array $config): bool => $config['default_connection'] && !\array_key_exists($config['default_connection'], $config['connections']))
