...
class Indexer
{
    ...
    public function scheduleIndex($index, Document|ElasticaDocument $document)
    {
    ...
    public function scheduleUpdate($index, Document|ElasticaDocument $document)
    {
        $document->setIndex($index instanceof Index ? $index->getName() : $index);
        $this->updateDocumentData($document);
    ...
    public function scheduleCreate($index, Document|ElasticaDocument $document)
    {
        $document->setIndex($index instanceof Index ? $index->getName() : $index);
        $this->updateDocumentData($document);
    ...
    private function updateDocumentData($document): void
    {
        if ($document instanceof Document) {
            $context = $this->contextBuilder->buildContext(\get_class($document->getDto()));
            $data = $this->serializer->serialize($document->getData(), 'json', $context);
            $document->setData($data);
        } elseif ($document instanceof ElasticaDocument) {
